...
func TestFunctionSignature_ReadHead(t *testing.T) {
	hs, _ := types.NewHexString(inputDeliverToken)
...
func TestFunctionSignature_DecodeFunctionInputWithDeliverTokens(t *testing.T) {
	...
	// check content
	expectedAddress, _ := new(types.EtherAddress).FromString("0x499d4aab8acab015d319ebfe476e4c800079ce87")
	if err := CompareEtherAddress(expectedAddress, param0.Value.(*types.EtherAddress)); err != nil {
		t.Error(err)
		return
	}
...
func CompareEtherAddress(expected *types.EtherAddress, actual *types.EtherAddress) error {
